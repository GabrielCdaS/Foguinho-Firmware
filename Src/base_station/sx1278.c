/**
 * @file sx1278.c
 * @brief Adaptacao da estacao base ao driver oficial Semtech SX1276/77/78/79.
 */
...
static void ao_tx_concluido(void)
{
    ...
    (void)sx1278_modo_recepcao_continua();
}
...
status_t sx1278_inicializar(void)
{
    ...
    configurar_tx();
    inicializado = true;
    return sx1278_modo_recepcao_continua();
}
...
status_t sx1278_receber(uint8_t *buffer, uint8_t *tamanho, uint32_t timeout_ms)
{
    ...
    *tamanho = 0U;
    ...
    do {
        ...
        if (resultado != STATUS_OK && timeout_ms != 0U) return resultado;
...
status_t sx1278_configurar_lora(sx1278_sf_t sf, sx1278_bw_t bw, sx1278_cr_t cr)
{
    ...
    if (inicializado) {
        configurar_rx(true);
        configurar_tx();
        if (recepcao_ativa) (void)sx1278_modo_recepcao_continua();
    }
    return STATUS_OK;
}
...
bool sx1278_pacote_disponivel(void)
{
    ...
    if (!recepcao_ativa && !transmissao_ativa) {
        (void)sx1278_modo_recepcao_continua();
    }
